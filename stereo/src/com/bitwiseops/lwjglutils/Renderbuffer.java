...
public class Renderbuffer {
        ...
        try {
            if(!GLContext.getCapabilities().GL_ARB_framebuffer_object) {
                throw new RuntimeException(
                        "Renderbuffer objects are not supported.");
            }
            ...
            ARBFramebufferObject.glRenderbufferStorage(
                    ARBFramebufferObject.GL_RENDERBUFFER, internalFormat,
                    width, height);
            this.width = width;
            this.height = height;
        } catch(Exception e) {
    ...
    public void bind() {
        ARBFramebufferObject.glBindRenderbuffer(
                ARBFramebufferObject.GL_RENDERBUFFER, id);
    }

    public static void bindNone() {
        ARBFramebufferObject.glBindRenderbuffer(
                ARBFramebufferObject.GL_RENDERBUFFER, 0);
    }
